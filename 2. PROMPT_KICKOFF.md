...
- Enforce the N/A rule from project_description.md §5.1: if a value in project_description.md or resources.md is `N/A`, the responsible agent decides without re-asking the user, documents the decision in their deliverable, and flags it in task_board.md. Surface every N/A-derived decision in the interim/final report.
- Before spawning Designer or DevOps agents: verify the MCP servers they need are available. If a value in resources.md is still `[PLACEHOLDER]`, ask the user once and write the answer back to resources.md. If they say `N/A`, treat per the rule above.
